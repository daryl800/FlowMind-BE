...
def calc_bazi_fixed(dt: datetime, tz_name: str, longitude: float):
    # Keep original (2) logic for calculation
    
    # 1️⃣ timezone normalize (from original (2))
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=ZoneInfo(tz_name))
    else:
        dt = dt.astimezone(ZoneInfo(tz_name))

    # 2️⃣ Calculate true solar time (from original (2))
    true_solar = calculate_true_solar_time(dt, longitude)

    # 3️⃣ Zi-hour day switch (from original (2), but modified to match (1)'s >=23 logic)
    solar_date = true_solar.date()
    if true_solar.hour >= 23:  # Changed from == 23 to >= 23 to match (1)
        solar_date += timedelta(days=1)

    # 4️⃣ Get lunar date (from original (2))
    lunar = sxtwl.fromSolar(solar_date.year, solar_date.month, solar_date.day)

    # 5️⃣ Get year, month, day pillars (from original (2))
    ...
    # 6️⃣ Get hour pillar using 五鼠遁 (from original (2))
    hour_zhi = get_hour_zhi(true_solar.hour)  # Uses true solar time hour!
    ...
    # 7️⃣ Build pillars structure to match (1)'s format
    pillars = {
    ...
    # 8️⃣ Day Master (from (1))
    day_master = pillars["day"]["gan"]

    # 9️⃣ Five Elements Strength (from (1))
    ELEMENT_MAP = {
        "Jia":"Wood", "Yi":"Wood",
        "Bing":"Fire","Ding":"Fire",
        "Wu":"Earth","Ji":"Earth",
    ...
    # - Includes additional info from (2)
    return {
        # Core structure matching (1)
        "pillars": pillars,
        "day_master": day_master,
        "five_elements_strength": fe_strength,
        
        # # Also include direct pillar keys for test compatibility
        # "year": pillars["year"],
        # "month": pillars["month"],
        # "day": pillars["day"],
        # "hour": pillars["hour"],
        
        # # Additional info from (2)
...
def calc_bazi(dt: datetime, tz_name: str = "Hong_Kong"):
    ...
    bazi_result = calc_bazi_fixed(dt=dt, tz_name=tz_resolved, longitude=longitude)
    return bazi_result
...
def enrich_with_localized_pillars(bazi: dict, lang="cn"):
    ...
    # Handle the structure that calc_bazi_fixed now returns
    # It has "pillars" and also direct "year", "month", "day", "hour" keys
    if "pillars" in bazi:
        pillars = bazi["pillars"]
        # Also update the direct keys if they exist
        ...
        pillars = {k: bazi[k] for k in ["year", "month", "day", "hour"] if k in bazi}
    
    # Add labels to pillars
    for key, gz in pillars.items():
        g = gz["gan"]
        z = gz["zhi"]
        gz["label"] = gan_map[GAN.index(g)] + zhi_map[ZHI.index(z)]
